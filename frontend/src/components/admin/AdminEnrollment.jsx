...
const AdminEnrollment = () => {
    ...
    // 5. Logic tính toán sĩ số (Khớp với Key Backend: current_enrolled)
    const currentClassInfo = classes.find(c => String(c.id) === String(selectedClass));
    const currentCount = currentClassInfo?.current_enrolled || 0; 
                    ...
                    >
                        ...
                        {classes.map(cls => (
                            <option key={cls.id} value={cls.id}>
                                {cls.class_name} (Sĩ số: {cls.current_enrolled || 0}/{cls.max_capacity || 25})
                            </option>
                        ))}
                    </select>
                </div>
